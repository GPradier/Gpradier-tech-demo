...
$(function() {
	...
	/*
	$("#dropzone").dropzone({ url: '/favstory/new/posts/edit/<?=$post['_id']?>' });
	...
	*/
	...
	//Dropzone.options.formDropzone = { maxFilesize: 1, paramName: 'upload'};
	
	/*
	var myDropzone = $("#form-dropzone").dropzone({maxFilesize: 1, paramName: 'upload'});
	myDropzone.on("error", function(file, message) { alert(message); });
	*/
	
	
	/*
	var formDropzone = $("#form-dropzone").dropzone({maxFilesize: 1, paramName: 'upload', accept:  function(file, done) {alert(file);done();} });
	formDropzone.on("error", function(file, message) { alert(message); });
	*/
	
	
	/*
	var myDropzone = $("#form-dropzone").data("dropzone");
	$("#form-dropzone").dropzone({maxFilesize: 1, paramName: 'upload', accept:  function(file, done) {alert(file);done();} });
	
	window.Dropzone.options.formDropzone = { thumbnailWidth: 200, maxFilesize: 10, paramName: 'upload', accept: function(file, done) {
	alert('hi');
	done("mh?");
  }};
	myDropzone.on("error", function(file, message) { alert(message); });
...
<?php
if($currentStep == 'etape1' || $currentStep == null) {
	echo '<style>.etape1{display:block} .etape2{display:none} .etape3{display:none}</style>';
}
if($currentStep == 'etape2') {
	echo '<style>.etape1{display:none} .etape2{display:block} .etape3{display:none}</style>';
}
if($currentStep == 'etape3') {
	echo '<style>.etape1{display:none} .etape2{display:none} .etape3{display:block}</style>';
}
if($currentStep == 'etape4') {
	echo '<style>.etape1{display:none} .etape2{display:none} .etape3{display:none}</style>';
}

if($currentStep == 'dev') {
	echo '<style>.dev{display:block}</style>';
} else {echo '<style>.dev{display:none}</style>';}
?>

<div class="dev" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(32,32,32,0.45);z-index:99">
